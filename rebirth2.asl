...
startup
{
    ...
    vars.getEXPBytes = (Func<Process, int, int, byte[]>) ((mem, saveBlock, slot) => {
		return mem.ReadBytes((System.IntPtr)(saveBlock + 0xF30 + 0x28 + 0x508*slot), 4);
	});
	
	vars.setSEXPBytes = (Func<Process, int, int, byte[], bool>) ((mem, saveBlock, slot, EXP) => {
		mem.WriteBytes((System.IntPtr)(saveBlock + 0xF30 + 0x28 + 0x508*slot), EXP);
		return true;
	});
    ...
    settings.Add("exp0", false, "Set EXP to 0", "memoryedits");
...
update
{
	...
	if (settings["memoryedits"]) {
        ...
        if (settings["exp0"]) {
            for(int i=0;i<23;i++) {
                vars.setSEXPBytes(memory, current.SaveBlock, i, new byte[] {(byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x80 });
            }
        }
	}
}
