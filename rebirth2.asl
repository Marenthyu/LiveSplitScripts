...
startup
{
	...
	vars.nepgearCutscenes.Add("Ch. 3 - Nepgear and Uni",  "");
...
update
{
	...
	if(settings["startingparty"]) {
		try {
			...
			String[] delGearCutscenes = new String[] { "Ch. 1 - Planeptune's Mascot", "Ch. 2 - Good-bye, Uni", "Ch. 2 - Uni's Defeat", "Ch. 3 - CFW Brave"};
			String[] delUniCutscenes = new String[] { "Ch. 2 - Good-bye, Uni", "Ch. 3 - CFW Brave"};
			String[] introCutscenes = new String[] { "New Game", "Prelude to the End", "Prologue - Gamindustri Graveyard", "Prologue - Escaping Judgment", "Prologue - The Trio Escape", "Ch. 1 - Divine Oratorio"};
			...
			if(!settings["char5"]) {
				...
				if(System.Array.Exists(delGearCutscenes, element => element.Equals(current.Cutscene)) && !old.Cutscene.Equals(current.Cutscene)) {
					// re-disable nepgear
					int ngIndex = vars.dataIndexOfCharacter(memory, current.SaveBlock, 6);
			...
			if(!settings["char6"]) {
				...
				if(System.Array.Exists(delUniCutscenes, element => element.Equals(current.Cutscene)) && !old.Cutscene.Equals(current.Cutscene)) {
					int uIndex = vars.dataIndexOfCharacter(memory, current.SaveBlock, 8);
			...
			if(!vars.inGhostNepgear && !vars.inGhostUni && !System.Array.Exists(introCutscenes, element => element.Equals(current.Cutscene))) {
				int dataIdx = 0;
